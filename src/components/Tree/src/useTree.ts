...
export function useTree(treeDataRef: Ref<TreeDataItem[]>, getFieldNames: ComputedRef<FieldNames>) {
   ...
   * 根据节点ID获取节点内容
   */
  function getNodeTextByKey(key: string, list?: TreeDataItem[]) {
    if (!key) return '';
    let nodeText = '';
    const treeData = list || unref(treeDataRef);
    const { key: keyField, title: titleField, children: childrenField } = unref(getFieldNames);
    if (!childrenField || !keyField || !titleField) return '';
    ...
    for (let index = 0; index < treeData.length; index++) {
      ...
      if (element[keyField] === key) {
        nodeText = element[titleField];
        break;
      } else if (children && children.length) {
        nodeText = getNodeTextByKey(key, element[childrenField]);
        if (nodeText != '') {
          break;
        }
      }
   ...
   * 获取父节点ID
   */
  function getParentIdByKey(key: string, parentId?: string, list?: TreeDataItem[]) {
    if (!key) return '';
    let rtnId = '';
    const treeData = list || unref(treeDataRef);
    const { key: keyField, title: titleField, children: childrenField } = unref(getFieldNames);
    ...
    for (let index = 0; index < treeData.length; index++) {
      ...
      if (element[keyField] === key) {
        rtnId = parentId ? parentId : '';
        break;
      } else if (children && children.length) {
        rtnId = getParentIdByKey(key, element[keyField], element[childrenField]);
        if (rtnId != '') {
          break;
        }
      }
